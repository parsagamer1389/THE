...
#==================== Config =====================#
Admin = 8103067280
Token = "8783265424:AAFTP5QRCLOXYTmFHI3vYhkV74c3PaDfDQM"
API_ID = 32723346
API_HASH = "00b5473e6d13906442e223145510676e"
Channel_ID = "NexSelf"
Channel_Help = "NexSelf_Help"
Helper_ID = "SHAH_SELF"
DBName = "SELFSAZ"
api_channel = "NexSelf"
DBUser = "SELFSAZ"
DBPass = "S1234567"
HelperDBName = "SELF_SAZ"
HelperDBUser = "SELF_SAZ"
HelperDBPass = "S1234567"
...
def get_pool_status():
    """دریافت وضعیت فعلی پول دیتابیس"""
    status = {
        "max_connections": db_pool._maxconnections,
        "min_cached": db_pool._mincached,
        "max_cached": db_pool._maxcached,
        "blocking": db_pool._blocking,
        "max_usage": db_pool._maxusage,
        "connections_in_use": 0,
        "available_connections": 0,
        "cached_connections": len(db_pool._cache),
        "total_created": len(db_pool._connections),
    }
...
def get_database_stats():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # تعداد جدول‌ها
            cursor.execute("SELECT COUNT(*) as count FROM information_schema.tables WHERE table_schema = %s", [DBName])
        ...
        return {
            ...
            "db_size": db_size if db_size else 0,
...
def optimize_database_tables():
    ...
    tables = ['user', 'cards', 'codes', 'settings', 'payment_transactions', 
              'resellers', 'user_reseller', 'pending_customers', 'reseller_transactions']
        ...
        with connection.cursor() as cursor:
            for table in tables:
                try:
                    ...
                    result = cursor.fetchone()
                    results[table] = "موفق" if result else "ناموفق"
                except:
                    results[table] = "خطا"
        connection.commit()
    except Exception as e:
...
def get_slow_queries():
    """دریافت کوئری‌های کند (در صورت فعال بودن slow log)"""
    slow_queries = []
    try:
        connection = db_pool.connection()
        with connection.cursor() as cursor:
            ...
            if is_enabled and is_enabled.get('Value') == 'ON':
                cursor.execute("SHOW VARIABLES LIKE 'long_query_time'")
                long_time = cursor.fetchone()
...
def reset_db_pool():
    ...
    try:
        ...
        return True, "پول دیتابیس با موفقیت ریست شد"
    except Exception as e:
        return False, str(e)[:100]
...
AdminSettingsKeyboard = InlineKeyboardMarkup(
    [
		...
		[InlineKeyboardButton("🗄 مدیریت دیتابیس", callback_data="DBPoolManagement")]
...
• بعد از پرداخت بر روی دکمه اعتبار سنجی بزنید.
    ...
    elif data == "DBPoolManagement":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            pool_status = get_pool_status()
            is_connected, conn_msg = test_database_connection()
            
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("📊 وضعیت پول", callback_data="PoolStatus")],
                [InlineKeyboardButton("📈 آمار دیتابیس", callback_data="DatabaseStats")],
                [InlineKeyboardButton("🔄 تست اتصال", callback_data="TestDBConnection")],
                [InlineKeyboardButton("⚙️ بهینه‌سازی جداول", callback_data="OptimizeDBTables")],
                [InlineKeyboardButton("🔄 ریست پول دیتابیس", callback_data="ResetDBPool")],
                [InlineKeyboardButton("⚡ تنظیمات پول", callback_data="PoolSettings")],
                [InlineKeyboardButton("🔙 بازگشت", callback_data="AdminPanel")]
            ])
            
            status_text = "🟢 فعال" if is_connected else "🔴 قطع"
            
            await app.edit_message_text(chat_id, m_id, f"**• مدیریت Database Connection Pool :**\n\n**🔄 وضعیت اتصال :** {status_text}\n**📝 پیام :** {conn_msg}\n\n**📊 تعداد پول‌های کش شده :** {pool_status.get('cached_connections', 0)}\n**📈 کل اتصالات ایجاد شده :** {pool_status.get('total_created', 0)}\n\n**• برای مشاهده جزئیات بیشتر روی دکمه‌ها کلیک کنید.**", reply_markup=keyboard)
    
    elif data == "PoolStatus":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            pool_status = get_pool_status()
            
            status_text = f"""**• وضعیت Connection Pool :**
    
    **⚙️ تنظیمات :**
    **🔗 حداکثر اتصالات :** {pool_status['max_connections']}
    **💾 حداقل اتصالات کش :** {pool_status['min_cached']}
    **📦 حداکثر اتصالات کش :** {pool_status['max_cached']}
    **🚦 حالت Blocking :** {pool_status['blocking']}
    **🔁 حداکثر استفاده :** {pool_status['max_usage']}
    
    **📊 وضعیت فعلی :**
    **🗄 اتصالات کش شده :** {pool_status['cached_connections']}
    **📈 کل اتصالات ایجاد شده :** {pool_status['total_created']}
    
    **• اتصالات در حال استفاده :** {pool_status['connections_in_use']}
    """
            await app.edit_message_text(chat_id, m_id, status_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
    
    elif data == "DatabaseStats":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            stats = get_database_stats()
            
            if stats:
                # تبدیل سایز به واحد مناسب
                size_bytes = stats['db_size']
                if size_bytes < 1024:
                    size_text = f"{size_bytes} بایت"
                elif size_bytes < 1024 * 1024:
                    size_text = f"{size_bytes / 1024:.2f} کیلوبایت"
                elif size_bytes < 1024 * 1024 * 1024:
                    size_text = f"{size_bytes / (1024 * 1024):.2f} مگابایت"
                else:
                    size_text = f"{size_bytes / (1024 * 1024 * 1024):.2f} گیگابایت"
                
                stats_text = f"""**• آمار دیتابیس :**
    
    **📊 آمار کلی :**
    **📋 تعداد جدول‌ها :** {stats['tables_count']}
    **💾 حجم دیتابیس :** {size_text}
    
    **👥 آمار رکوردها :**
    **👤 کاربران :** {stats['users_count']:,}
    **💳 کارت‌ها :** {stats['cards_count']:,}
    **💰 تراکنش‌ها :** {stats['transactions_count']:,}
    
    **• دیتابیس در حال کار عادی است.**
    """
            else:
                stats_text = "**• خطا در دریافت آمار دیتابیس**"
            
            await app.edit_message_text(chat_id, m_id, stats_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
    
    elif data == "TestDBConnection":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• در حال تست اتصال به دیتابیس، لطفا صبر کنید...**")
            
            results = []
            
            # تست اتصال اصلی
            is_connected, msg = test_database_connection()
            results.append(f"**اتصال اصلی :** {'✅ موفق' if is_connected else '❌ ناموفق'} - {msg}")
            ...
            results.append(f"**⏱ زمان پاسخگویی :** {query_time:.2f} میلی‌ثانیه")
            
            # تست Helper DB
            try:
                helper_conn = helper_db_pool.connection()
                with helper_conn.cursor() as cursor:
                    cursor.execute("SELECT 1")
                helper_conn.close()
                results.append("**🟢 Helper DB :** ✅ متصل")
            except:
                results.append("**🟡 Helper DB :** ❌ قطع")
            
            await app.edit_message_text(chat_id, m_id, "\n".join(results), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
    
    elif data == "OptimizeDBTables":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• در حال بهینه‌سازی جداول دیتابیس، لطفا صبر کنید...**")
            
            results = optimize_database_tables()
            
            result_text = "**• نتیجه بهینه‌سازی جداول :**\n\n"
            for table, status in results.items():
                emoji = "✅" if status == "موفق" else "❌" if status == "ناموفق" else "⚠️"
                result_text += f"{emoji} **{table} :** {status}\n"
            
            await app.edit_message_text(chat_id, m_id, result_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
    
    elif data == "ResetDBPool":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ بله، ریست کن", callback_data="ConfirmResetPool")],
                [InlineKeyboardButton("❌ خیر، انصراف", callback_data="DBPoolManagement")]
            ])
            
            await app.edit_message_text(chat_id, m_id, "**• آیا از ریست کردن پول دیتابیس مطمئن هستید؟**\n\n**• تمام اتصالات فعلی بسته خواهند شد و پول بازسازی می‌شود.**", reply_markup=keyboard)
    
    elif data == "ConfirmResetPool":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• در حال ریست کردن پول دیتابیس، لطفا صبر کنید...**")
            
            success, msg = reset_db_pool()
            
            if success:
                await app.edit_message_text(chat_id, m_id, f"**✅ {msg}**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
            else:
                await app.edit_message_text(chat_id, m_id, f"**❌ خطا : {msg}**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]]))
    
    elif data == "PoolSettings":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔗 تغییر max_connections", callback_data="SetMaxConnections")],
                [InlineKeyboardButton("💾 تغییر min_cached", callback_data="SetMinCached")],
                [InlineKeyboardButton("📦 تغییر max_cached", callback_data="SetMaxCached")],
                [InlineKeyboardButton("🔁 تغییر max_usage", callback_data="SetMaxUsage")],
                [InlineKeyboardButton("🔙 بازگشت", callback_data="DBPoolManagement")]
            ])
            
            pool_status = get_pool_status()
            
            await app.edit_message_text(chat_id, m_id, f"**• تنظیمات Connection Pool :**\n\n**🔗 max_connections :** {pool_status['max_connections']}\n**💾 min_cached :** {pool_status['min_cached']}\n**📦 max_cached :** {pool_status['max_cached']}\n**🔁 max_usage :** {pool_status['max_usage']}\n\n**• تغییرات پس از ریست پول اعمال می‌شوند.**", reply_markup=keyboard)
    
    elif data == "SetMaxConnections":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• لطفا عدد جدید برای max_connections وارد کنید (مثال : 150) :**\n\n**• مقدار پیشفرض : 100**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            update_data(f"UPDATE user SET step = 'set_max_connections' WHERE id = '{chat_id}'")
    
    elif data == "SetMinCached":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• لطفا عدد جدید برای min_cached وارد کنید (مثال : 20) :**\n\n**• مقدار پیشفرض : 30**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            update_data(f"UPDATE user SET step = 'set_min_cached' WHERE id = '{chat_id}'")
    
    elif data == "SetMaxCached":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• لطفا عدد جدید برای max_cached وارد کنید (مثال : 80) :**\n\n**• مقدار پیشفرض : 50**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            update_data(f"UPDATE user SET step = 'set_max_cached' WHERE id = '{chat_id}'")
    
    elif data == "SetMaxUsage":
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id, "**• لطفا عدد جدید برای max_usage وارد کنید (مثال : 1000) :**\n\n**• مقدار پیشفرض : 2000\n• 0 یعنی بدون محدودیت**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            update_data(f"UPDATE user SET step = 'set_max_usage' WHERE id = '{chat_id}'")
        ...
        if verify_result["success"]:
            ...
            success_message = f"""**• پرداخت با موفقیت انجام شد.
...
async def contact_handler(c, m):
    ...
    elif user.get("step") == "set_max_connections":
        if chat_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{chat_id}' LIMIT 1") is not None:
            if text.isdigit() and int(text) > 0:
                # ذخیره تنظیمات (در دیتابیس یا فایل)
                update_setting("db_max_connections", text)
                await app.send_message(chat_id, f"**• مقدار max_connections به {text} تنظیم شد.**\n\n**• برای اعمال تغییرات، پول دیتابیس را ریست کنید.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            else:
                await app.send_message(chat_id, "**• لطفا یک عدد معتبر وارد کنید.**")
            update_data(f"UPDATE user SET step = 'none' WHERE id = '{chat_id}'")
    
    elif user.get("step") == "set_min_cached":
        if chat_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{chat_id}' LIMIT 1") is not None:
            if text.isdigit() and int(text) >= 0:
                update_setting("db_min_cached", text)
                await app.send_message(chat_id, f"**• مقدار min_cached به {text} تنظیم شد.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            else:
                await app.send_message(chat_id, "**• لطفا یک عدد معتبر وارد کنید.**")
            update_data(f"UPDATE user SET step = 'none' WHERE id = '{chat_id}'")
    
    elif user.get("step") == "set_max_cached":
        if chat_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{chat_id}' LIMIT 1") is not None:
            if text.isdigit() and int(text) >= 0:
                update_setting("db_max_cached", text)
                await app.send_message(chat_id, f"**• مقدار max_cached به {text} تنظیم شد.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            else:
                await app.send_message(chat_id, "**• لطفا یک عدد معتبر وارد کنید.**")
            update_data(f"UPDATE user SET step = 'none' WHERE id = '{chat_id}'")
    
    elif user.get("step") == "set_max_usage":
        if chat_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{chat_id}' LIMIT 1") is not None:
            if text.isdigit() and int(text) >= 0:
                update_setting("db_max_usage", text)
                await app.send_message(chat_id, f"**• مقدار max_usage به {text} تنظیم شد.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="PoolSettings")]]))
            else:
                await app.send_message(chat_id, "**• لطفا یک عدد معتبر وارد کنید.**")
            update_data(f"UPDATE user SET step = 'none' WHERE id = '{chat_id}'")
...
def initialize_settings():
    ...
    default_db_settings = [
        ("db_max_connections", "100", "حداکثر اتصالات دیتابیس"),
        ("db_min_cached", "30", "حداقل اتصالات کش شده"),
        ("db_max_cached", "50", "حداکثر اتصالات کش شده"),
        ("db_max_usage", "2000", "حداکثر استفاده از هر اتصال"),
    ]
    
    for key, value, description in default_db_settings:
        if get_data(f"SELECT * FROM settings WHERE setting_key = '{key}'") is None:
            update_data(f"INSERT INTO settings(setting_key, setting_value, description) VALUES('{key}', '{value}', '{description}')")
